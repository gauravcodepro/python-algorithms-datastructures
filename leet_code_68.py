...
#boolean query(char letter) Accepts a new character from the stream and returns true if 
...
[(i,'True') if i=='input' in a else 'False' for i in [i[1] 
         for i in (list(filter(lambda n: len(n)>1,list(map(lambda n: list(n),steamChecker)))))]]  