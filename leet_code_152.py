...
list(filter(lambda n: min(n)+max(n) <=9,list(filter(None,[nums[::i] for i in range(1,len(nums))]+
           [nums[i:i+i] for i in range(1,len(nums))]+[nums[:i] for i in range(len(nums))]))))