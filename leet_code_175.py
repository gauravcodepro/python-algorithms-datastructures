...
['True' if sum([nums[i:i+2] for i in range(len(nums)-(2-1))][i])==
      sum([nums[i:i+2] for i in range(len(nums)-(2-1))][i+1]) else 'False' 
            for i in range(len([nums[i:i+2] for i in range(len(nums)-(2-1))])-1)]